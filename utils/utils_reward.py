...
def compute_mean_std(states: np.ndarray, eps: float = 0.0) -> Tuple[np.ndarray, np.ndarray]:
    mean = states.mean(0)
    std = states.std(0) + eps
    std[std < 1e-12] = 1.0 # Judge if std is small 
    return mean, std
...
def obtain_labels(dataset, idx_1, idx_2, segment_size=25, threshold=0.01, noise=0.0):
    ...
    equal_labels = np.where(
        np.abs(reward_1 - reward_2) <= gap, 1, 0
    )
    labels = np.array(
