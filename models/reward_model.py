...
class RewardModel:
    def __init__(self,
                 config,
                 obs_act_1: np.ndarray,
                 obs_act_2: np.ndarray,
                 labels: np.ndarray,
                 dimension: int,
                 logger):
        self.env = config.env
    ...
    def load_model(self, path):
        ...
        for member in range(self.ensemble_num):
            member_path = os.path.join(path, "reward_" + str(member) + ".pt")
            self.ensemble_model[member].load_state_dict(torch.load(member_path, map_location=self.device))

    def get_reward(self, dataset):
        obs = dataset["observations"]
        act = dataset["actions"]
        if "rewards" not in dataset:
            dataset["rewards"] = np.zeros(obs.shape[0])
    ...
    def eval(self, obs_act_1, obs_act_2, labels, binary_labels, name, epoch):
        ...
        self.logger.record("eval/acc", eval_acc, epoch)
        self.logger.record("eval/loss", eval_loss, epoch)
    ...
    def train_model(self):
        ...
        for epoch in tqdm.tqdm(range(self.epochs)):
            ...
            if epoch % 20 == 0:
                self.logger.record("train/loss", train_loss, epoch)
